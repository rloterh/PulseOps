  ...
  try {
    ...
    if (!canViewAnalytics(context.membershipRole)) {
      throw new SafeError({
        code: 'FORBIDDEN',
        status: 403,
        userMessage: 'You do not have permission to view analytics AI output.',
      });
    }
    ...
    if (error) {
      throw new Error('Failed to load analytics locations.');
    }
    ...
    log(error instanceof SafeError ? 'warn' : 'error', {
      message: 'Failed to load analytics AI overview.',
      context: {
        tenantId: context.tenantId,
        viewerId: context.viewerId,
