  ...
  try {
    ...
    if (!canViewAnalytics(context.membershipRole)) {
      throw new SafeError({
        code: 'FORBIDDEN',
        status: 403,
        userMessage: 'You do not have permission to view branch analytics.',
      });
    }
    ...
    if (error) {
      throw new Error('Failed to load branch analytics scope.');
    }
    ...
    log(error instanceof SafeError ? 'warn' : 'error', {
      message: 'Failed to load branch analytics.',
      context: {
        tenantId: context.tenantId,
        viewerId: context.viewerId,
