...
import type { TaskTimelineEntry } from '@/features/tasks/types/task.types';

export function TaskTimeline({ items }: { items: TaskTimelineEntry[] }) {
  return (
    <section className="rounded-[1.8rem] border border-white/8 bg-white/[0.04] p-5 shadow-[0_20px_70px_rgba(2,6,23,0.24)]">
      <div className="mb-5">
        ...
        <p className="mt-2 text-sm leading-6 text-white/46">
          Follow-up work is easier to coordinate when intake and ownership changes stay visible.
        </p>
      </div>
      <div className="space-y-4">
        {items.length > 0 ? (
          ...
          <div className="rounded-[1.35rem] border border-dashed border-white/10 bg-black/12 p-5 text-sm leading-6 text-white/52">
            No timeline events have been recorded for this task yet.
          </div>
        )}
      </div>
    </section>
