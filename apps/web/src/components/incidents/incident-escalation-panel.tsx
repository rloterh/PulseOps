...
export function IncidentEscalationPanel({
  ...
  return (
    <section className="rounded-[1.6rem] border border-white/8 bg-white/[0.04] p-5">
      ...
      {canManage ? (
        <form action={createIncidentEscalationAction} className="mt-5 space-y-3">
            ...
            return (
              ...
              >
                ...
                (escalation.status === 'pending' || escalation.status === 'sent') ? (
                  <form action={acknowledgeIncidentEscalationAction} className="mt-4">
                    <input type="hidden" name="incidentId" value={incidentId} />
                    <input type="hidden" name="escalationId" value={escalation.id} />
                    <button
                      type="submit"
                      className="inline-flex rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-sm font-medium text-white transition hover:bg-white/[0.08]"
                    >
