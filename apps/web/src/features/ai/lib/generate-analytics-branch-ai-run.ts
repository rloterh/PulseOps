...
import type {
  AnalyticsBranchComparisonAiResult,
...
const TASK_KEY = 'analytics_branch_comparison';
const PROMPT_VERSION = 'sprint9-branch-v1';
const REUSE_WINDOW_MS = 15 * 60 * 1000;

export async function generateAnalyticsBranchAiRun(
...
): Promise<AnalyticsBranchComparisonAiResult> {
  ...
  if (reusableRun) {
    ...
    return {
      ...
      summary: parseStoredSummary(reusableRun.response_payload, input.summary),
    };
  }
  ...
  try {
    const completedRun = await completeAiRunInDb(supabase, {
      ...
      provider: providerConfig.provider,
      model: providerConfig.model,
      responsePayload: input.summary as unknown as Json,
    ...
    await failAiRunInDb(supabase, {
      runId: pendingRun.id,
      errorMessage: error instanceof Error ? error.message : 'Unknown branch AI generation failure.',
      fallbackReason:
        providerConfig.fallbackReason ?? 'Deterministic branch AI generation failed unexpectedly.',
    });
    ...
    return {
      generation: buildGenerationMeta({
        ...
        fallbackReason:
          ...
          'Focused branch AI generation failed, so the live view is using the in-memory fallback.',
        feedbackRating: null,
      }),
      summary: input.summary,
    };
...
function resolveProviderConfig() {
  ...
  if (env.AI_PROVIDER === 'openai' && env.OPENAI_API_KEY) {
    return {
      ...
      model: 'heuristic-branch-v1',
      fallbackReason:
        'External AI provider is configured, but branch-comparison synthesis stays deterministic on this branch for auditability.',
    };
  }
...
function parseStoredSummary(
  payload: Json | null,
  fallback: AnalyticsBranchComparisonSummary,
): AnalyticsBranchComparisonSummary {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
    return fallback;
  }

  return payload as unknown as AnalyticsBranchComparisonSummary;
}
